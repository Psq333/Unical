...
def draw_text(str:str, x:int, y:int) -> None:
    ...
    Disegna la stringa str. Disegna su (x, y) 
    """
    global screen
    global font
...
def draw_block(num:int, x:int, y:int) -> None:
    ...
    Disegna un blocco colore col scritto come num in (x, y) 
    """
    global screen
    global font
    r = 237*num % 256
...
def get_top_y(x:int) -> int:
    ...
    Restituisce la coordinata y del blocco in cima alla colonna x 
    """
    global field
...
def is_fallable(x:int, y:int) -> bool:
    ...
    Restituisce true se ci sono 0 blocchi sotto il campo (x, y) 
    """
    global field
...
def set_animation():
    ...
    Imposta l'animazione in base allo stato corrente del campo 
    """
...
def is_animating_place(x:int, y:int) -> bool:
    ...
    Restituisce true se le coordinate (x, y) sul campo sono in animazione 
    """
    global animation_directions
    global animation_place
    global animation_type
...
def main():
    ...
    # ブロックの数値データを格納するリスト(Un elenco che memorizza i dati numerici del blocco)
    field = [[0 for i in range(FIELD_W)] for j in range(FIELD_H)]
    score = 0

    # ----------操作中のブロックに関する変数 (Variabili relative al blocco manipolato)----------
    is_moving = False # 操作中か否か
    moving_xy = [-1, -1] # 座標 (Coordinate)
    moving_num = 0 # ブロックの数字 (Blocca i numeri)
    fall_spd = 0.08 # 落ちるスピード (Velocità)
    add_delay = 10 # 次のブロックが追加されるまでの時間 (Tempo fino all'aggiunta del blocco successivo)
    next_moving_num = rnd(0, 5) # 次にくるブロックの数字 (Il numero del blocco successivo)
    horizontal_input = 0 # 水平方向のキー入力 (Inserimento chiave orizzontale)
    horizontal_delay = 0 # 再度水平方向の操作が可能になるまでの時間 (Tempo prima che il funzionamento orizzontale sia nuovamente possibile)
    down_input = 65 # 下方向のキー入力

    # ----------アニメーションに関する変数 (Variabili relative all'animazione)----------
    ...
    while (running): 
        ...
        # キー取得 (Prendi la tasto cliccato)
        pressed = pygame.key.get_pressed()

        # キー入力 (Inserimento chiave)
        if horizontal_delay > 0:
            horizontal_delay -= 1
        if horizontal_delay == 0 and is_moving:
            #colonna = FIELD_W-utility.getSoluzione()
            #print("La colonna é " + str(colonna))
            #horizontal_input = colonna-1
            if moving_xy[0] < utility.getSoluzione():
                horizontal_input +=1
            elif moving_xy[0] > utility.getSoluzione():
                horizontal_input -=1
            else:
                down_input = 1
            '''
        ...
        # ゲームの数値処理 (Elaborazione numerica del gioco)
        if not game_over:
            if not is_animating:
                if is_moving:
                    # キーでの移動 (Muoversi con il tasto)
                    ...
                    if moving_xy[1] < FIELD_H-1 and field[math.floor(moving_xy[1])+1][moving_xy[0]] == 0:
                        ...
                        if down_input: moving_xy[1] += 20 #velocità
                    ...
                    # 接地
                        ...
                        if get_top_y(moving_xy[0]) > 0:
                            ...
                            # もし最高まで積みあがっていたらゲームオーバー (Se è accumulato al massimo, il gioco è finito)
                            game_over = True
                        add_delay = 10
                        is_moving = False
                else:
                    if add_delay > 0:
                        ...
                        #AGGIORNAMETO DLV
                        matrice = Matrice(field, FIELD_H, FIELD_W)

                        #utility = UtilityDLV()
                        utility.setFatti(matrice.toString(), moving_num)

                        utility.trasferisciInDLV()
                        
            
            # アニメーション更新 (Aggiornamento dell'animazione)
            if is_animating:
                if animation_type == ANI_CONBINE:
